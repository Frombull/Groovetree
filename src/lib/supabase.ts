...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    persistSession: false,   // Don't persist sessions since we use custom JWT
    detectSessionInUrl: false // Don't detect auth callbacks in URL
  }
});
...
export async function deleteProfilePicture(url: string): Promise<boolean> {
  try {
    ...
    const bucketIndex = urlParts.findIndex(
      (part) => part === PROFILE_PICTURES_BUCKET
    );
